...
- Tool servers should import `FastMCP` via `from fastmcp import FastMCP`.
- Tool servers should support `stdio` and `http` transport modes.
- Tool servers should pass HTTP bind settings to `mcp.run(transport="http", host=HOST, port=PORT)`.
